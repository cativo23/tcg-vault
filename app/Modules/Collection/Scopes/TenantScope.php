...
final class TenantScope implements Scope
{
     ...
     * since `user_id` is NOT NULL. An earlier version of this scope only
     * filtered `if (auth()->check())`, which meant an unauthenticated
     * context (a bug in a route, a queued job with no session) silently
     * saw every tenant's rows instead of none. Code that genuinely needs
